...
[{if $actionAllowed}]
    <div class="form-block">
        ...
        <div style="padding: 2px 7px">
            <input type="button" name="[{$action}]-submit-btn" id="[{$action}]-submit-btn" class="form-submit-btn" style="padding: 3px;" value="[{oxmultilang ident="PAYEVER_ORDER_PROCESS_`$actionLang`"}]">
        </div>
        <script type="application/javascript">
            let invoiceForm = document.getElementById('invoice-amount-form');
            invoiceForm.addEventListener('submit', function () {
                let btn = invoiceForm.querySelector('.form-submit-btn');
                btn.disabled = true;
                btn.parentElement.classList.add('loader');
            });

            document.getElementById('invoice-submit-btn').addEventListener('click', function (e) {
